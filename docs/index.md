...
| [data-architecture.md](data-architecture.md)       | Three-layer data model, local-first data layer, personal KB (JSON schema), narrative notes                     | Fully implemented (local-first + 3 phrase types + KB + connections + familiarity + backlinks). Patterns (#14) TBD |
...
4. Data architecture has been decided (three layers) and mostly implemented (only Patterns remain). Three phrase match types: lemma, root, and surface.
