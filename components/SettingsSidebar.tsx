...
export const SettingsSidebar: React.FC<SettingsSidebarProps> = ({ 
  ...
  // Group by Language
  const groupedTranslations = filteredTranslations.reduce((groups, trans) => {
    ...
    <>
      ...
      `}>
        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
          <h2 className="font-bold text-lg text-emerald-600 dark:text-emerald-400 tracking-wider">SETTINGS</h2>
          <button onClick={onClose} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-400 hover:text-slate-700 dark:hover:text-white">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
            ...
            {expandedSection === 'font' && (
              <div className="px-5 pb-6 animate-in slide-in-from-top-2 duration-200">
                <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                  ...
                  />
                  <div className="mt-4 text-center font-quran text-slate-800 dark:text-white" style={{ fontSize: `${settings.fontSize}px` }}>
                    بِسْمِ ٱللَّهِ
                  </div>
                </div>
              </div>
              ...
              <div className="px-5 pb-6 animate-in slide-in-from-top-2 duration-200">
                <div className="space-y-2">
                   ...
                   >
                     ...
                     <span className="block text-xs font-sans text-left mt-2 text-slate-500 font-normal">Standard (Simple)</span>
                   </button>
                   ...
                   >
                     ...
                     <span className="block text-xs font-sans text-left mt-2 text-slate-500 font-normal">Uthmani (Original)</span>
                   </button>
                </div>
              </div>
            )}
          ...
          <div className="border-b border-slate-100 dark:border-slate-800">
            ...
            >
              ...
              {expandedSection === 'translations' ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
            </button>
            
            {expandedSection === 'translations' && (
              <div className="px-5 pb-6 animate-in slide-in-from-top-2 duration-200">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-xs font-medium text-slate-500 uppercase tracking-wide">Show Text</span>
                  <button 
                    onClick={() => onUpdateSettings({ showTranslation: !settings.showTranslation })}
                    className="text-emerald-500 hover:text-emerald-400 transition-colors"
                  >
                    {settings.showTranslation ? <ToggleRight size={28} /> : <ToggleLeft size={28} className="text-slate-300 dark:text-slate-600" />}
                  </button>
                </div>
                    ...
                    {loadingTranslations ? (
                      ...
                      <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                         {filteredTranslations.length === 0 && (
                           <div className="text-center py-4 text-slate-400 text-sm">No translations found</div>
                         )}
                           ...
                           <div key={lang} className="space-y-1">
                             ...
                             {groupedTranslations[lang].map(trans => {
                               ...
                               return (
                                 ...
                                 >
                                   <div className={`shrink-0 mt-0.5 ${isSelected ? 'text-emerald-600 dark:text-emerald-500' : 'text-slate-400 dark:text-slate-600'}`}>
                                     {isSelected ? <CheckSquare size={16} /> : <Square size={16} />}
                                   </div>
                                   <span className={`text-sm leading-snug ${isSelected ? 'text-slate-800 dark:text-white font-medium' : 'text-slate-500 dark:text-slate-400'} ${isRtlLang ? 'font-vazir' : 'font-sans'}`}>
                                     {trans.name}
                                   </span>
                                 </button>
                               );
