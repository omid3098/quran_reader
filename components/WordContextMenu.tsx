...
export const WordContextMenu: React.FC<WordContextMenuProps> = ({ position, word, onClose, onSelectVajehyab, onSelectAbjad }) => {
  ...
  return (
    ...
    >
        <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700 mb-1 bg-slate-50/50 dark:bg-slate-900/50">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Selected Word</span>
            <div className="font-quran text-xl text-emerald-600 dark:text-emerald-400 truncate mt-1 text-center">{word}</div>
        </div>
        <button 
            onClick={() => { onSelectVajehyab(); onClose(); }} 
